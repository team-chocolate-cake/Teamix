...
fun getArchivedSavedItem()
        : List<SavedItemOfDay> {
    return savedItemsList.filter { it.state == SavedItemState.ARCHIVED }.groupBy { it.date }
        .map {
            SavedItemOfDay(date = it.key, saveItems = it.value)
        }
}

fun getCompletedSavedItem()
        : List<SavedItemOfDay> {
    return savedItemsList.filter { it.state == SavedItemState.COMPLETED }.groupBy { it.date }
        .map {
            SavedItemOfDay(date = it.key, saveItems = it.value)
        }
}
